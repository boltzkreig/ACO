...
double *rel_optimize(double *data,long *shape,int size, int ants ,int iter, int neig, double sens){
	//double * c_arr		=calloc(size, sizeof(double));
	double * Phe_Mat	=calloc(size, sizeof(double));
	insect * ANT 		= calloc(ants,sizeof(insect));
	//srand(time(0));
...
// Iterations are run; each iteration goes through position of each ant
	for(n=0; n < iter; n++){
		for ( i=0; i < ants; i++){
			...
			/*for(x=curr_x-neig; x<=curr_x+neig; x++){for(y=curr_y-neig; y<=curr_y+neig; y++){
				if(pos(x, y,h,l)==-1) continue;
				sum+=data[pos(x,y,h,l)];	tally++;
				sum=sum/tally;
				}}
				if(fabs(data[pos(curr_x,curr_y,h,l)] - sum) >= sens){
					Phe_Mat[pos(curr_x,curr_y,h,l)]=sum;
				}*/
				for(x=curr_x-1; x<=curr_x+1; x++){for(y=curr_y-1; y<=curr_y+1; y++){
				if(pos(x, y,h,l)==-1) continue;
				if(fabs(data[pos(x,y,h,l)] - data[pos(curr_x,curr_y,h,l)]) <= sens*data[pos(curr_x, curr_y, h, l)]){
					tally++;
				}}}
			if(tally <= neig){
				Phe_Mat[pos(curr_x,curr_y,h,l)]=1.0;
